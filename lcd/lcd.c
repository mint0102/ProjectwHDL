...
void lcd_send_command(unsigned char cmd)	//Sends Command to LCD
{
	...
	LCD_PORT&=0x0f; // dk bang 4 bit cuoi
	LCD_PORT|=(cmd<<4);
	LCD_PORT|=(1 <<LCD_E);
	v_timer0(1);
...
void lcd_write_data(unsigned char data) 
{
    ...
    lcd_write_4bits(data<<4);//dich bit de write
    RS(CMD);
}
...
void lcd_print_str (const rom unsigned char *string)
{
    while (*string)  
    {
        lcd_print_char (*string++);//dich len tu char
    }
}
...
void DisplayLcdScreen2() //mat 2 chu ky moi day het ra
{
    unsigned char i;
    switch (statusLCD)
    {
